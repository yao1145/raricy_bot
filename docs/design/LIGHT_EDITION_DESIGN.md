...
状态：设计基线；L0 平台原型、L1 导入解耦与 L2 配置/凭据已实现（2026-09-22，
`light-edition` 分支），冻结冒烟通过，干净 Windows 验收未执行。日期：2026-09-22。
...
（D-125）。验证（本地）：哨兵测试确认密码与模型 Key 不出现在配置、草稿、快照与元数据里；
...
版本与父子句柄交接方式仍待 L5；Light 模块闭包已由 staging 构建与验证测试锁定；L5 记录
实测包体积、空闲/工作内存、冷启动耗时和长时间运行结果。当前没有这些测量值，不以“轻量”
宣传具体体积或资源占用。ZIP 首发是否签名、后续安装器发布方式也需在发行前说明，
