...
状态：设计基线；L0 平台原型已实现（2026-09-22，`light-edition` 分支），冻结冒烟通过，
干净 Windows 验收未执行。日期：2026-09-22。
...
主程序静态页/启停/退出与互斥体释放。§17.2 干净 Windows 验收仍未执行。

发行前仍需在各阶段落实的工程选择：L0 锁定 Windows API 绑定、最低平台版本与父子句柄交接方式；
L1 确认 Light 模块白名单及完整依赖闭包；L5 记录实测包体积、空闲/工作内存、冷启动耗时和长时间运行结果。
当前没有这些测量值，不以“轻量”宣传具体体积或资源占用。ZIP 首发是否签名、后续安装器发布方式也需在发行前说明，
不能承诺无系统信誉提示。上述选择不影响本文已确认的产品范围，也不要求此时启动实现。
