...
> 共同知识或多副本写入。
>
> **2026-09-17 追加**：本文只覆盖前两类记忆（共同记忆与用户私有记忆）。第三类
> 「用户公开个人记忆」另有一份设计与内部合同，**不并入本文**：产品行为见
> `docs/design/PUBLIC_PERSONAL_MEMORY_DESIGN.md`（含它自己的任务拆分与验收标准），接口小节见
> `INTERFACES.md` §39–§52，裁决见 `DESIGN_DECISIONS.md` D-96–D-103（其中 D-56、D-76 有 2026-09-17
> 的补充段落）。本文描述的 `MemoryConfig`、`MemoryScope`、命令表与评论 provider 都是那两类的
> 合同，读第三类时不要拿本文的字段表反推它的行为。
