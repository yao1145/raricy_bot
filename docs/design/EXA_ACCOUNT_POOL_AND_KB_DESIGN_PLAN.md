...
> 状态：**已实施**（正式合同见 `docs/design/INTERFACES.md` §22 / §23，裁决见 D-36 … D-46）
...
| ID | 结论 | 证据 / 处置 |
|---|---|---|
| P0-01 | **仍是部署门禁，代码已按「未获批则不启用」实现** | 池是可选配置，默认不存在；未配置 `account_pool` 时行为逐字节不变。部署指南 §1 第 5 条写明上线前必须拿到书面授权或改用官方容量方案。任何自动化都不构成授权 |
| P0-02 | **已按「无法分类就安全失败」关闭** | 分类器只在结构化字段与 `isError` 文本上做固定词匹配，其余归 `unknown_upstream` 且**不轮换**（D-46）。固定脱敏样例的契约测试在 `tests/test_mcp_pool.py`。**按额度自动轮换**仍要求拿到 3.4.1 的真实样例证据 |
| P0-03 | **已按私密默认关闭** | `enabled=false` / `access_mode=allowlist` / `allowed_channel_kinds=["dm"]`；公开必须显式配置（D-44）。「资料负责人签字」属部署侧人工事项，见部署指南 §1 第 6 条 |
| P0-04 | **目录清点仍是部署门禁** | 清点清单写进部署指南 §1 第 6 条与 README 上线检查清单；代码不承诺目录内容 |
| P0-05 | **已裁决并实现** | 能力轮次把 `context_input_tokens` 当硬上限，允许把历史整对丢光；普通聊天语义不变（D-38） |

其余 P1/P2 条目按文中「已建议」的默认执行；标为「待验证」的（同一 Team 预算归属、
429 的计费维度、多子进程资源开销、CJK 召回质量、Rocky SELinux 标签等）都保留为**目标环境验收项**，
代码不假装它们已经解决。
